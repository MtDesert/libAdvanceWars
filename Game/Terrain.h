...
struct TerrainCode{
	...
	bool capturable;//能否被占领,具有占领功能的兵种可以进行占领
	bool hasIncome;//是否具有收入,可占领的据点不一定会有收入
	bool buildable;//能否生产部队
	bool hidable;//是否可隐藏地形,隐藏地形只有靠近的时候才能发现内部情况
	bool has4direction;//地形是否具有4个方向,主要用于支持BattleField的调整图块功能
