...
uint Table_CorpsData::columnWidth(uint col)const{
...
void Table_CorpsData::render()const{
	/*GameTable::render();
	if(!source)return;
	//画数据
	rect=rectF();
	...
	for(auto &item:*source){
		if(index>=renderItemFrom && index < renderTo){//需要渲染的部分
			...
			if(textures){
				...
				auto tex=textures->value(item.id+troopID*256);
...
DataBlock Table_CorpsData::headerBlock(uint col)const{
